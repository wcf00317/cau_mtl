...
def _visualize_depth_task(model, batch, device, save_path):
    ...
    with torch.no_grad():
        ...
        # 1. 跑主解码器
        pred_main_raw = model.predictor_depth(main_feat, zp_depth_proj)

        # 2. [FIX] 加上遗漏的 Refiner 残差 (这才是完整的 f+zs+zp)
        zp_residual = model.zp_depth_refiner(zp_depth_proj)
        # 确保尺寸对齐 (Refiner 输出可能较小，需插值)
        if zp_residual.shape[-2:] != pred_main_raw.shape[-2:]:
            zp_residual = torch.nn.functional.interpolate(
                zp_residual, size=pred_main_raw.shape[-2:], mode='bilinear', align_corners=False
            )

        # 3. 最终相加
        pred_main = pred_main_raw + zp_residual
