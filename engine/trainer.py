...
def train(model, train_loader, val_loader, optimizer, criterion, scheduler, config, device,
    ...
    for epoch in range(total_epochs):
        ...
        if hasattr(real_criterion, 'weights'):
            # [保留] 唯一必要的追踪日志
            print(f"DEBUG Epoch {epoch}: Updating lambda_independence to {current_ind_lambda:.4f}")
        ...
        if val_loader_source is not None:
            # 这里的日志可以帮你监控是否过拟合源域
            logging.info(f"\n[Epoch {epoch + 1}] 🔍 Validating on SOURCE Domain (GTA5)...")

            # 强制指定 data_type='gta5'，让 evaluator 只关注分割指标，避免找深度/法线真值报错
            source_metrics = evaluate(model, val_loader_source, criterion, device, stage=stage, data_type='gta5')

            # 简单打印一下 mIoU 即可，不用存 best，仅仅用于观察
            s_miou = source_metrics.get('seg_miou', 0.0)
            s_acc = source_metrics.get('seg_pixel_acc', 0.0)
            logging.info(f"   >>> Source (GTA5) Metrics: mIoU={s_miou:.4f}, Pixel Acc={s_acc:.4f}")
        # --- Validate ---
